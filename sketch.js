...
sew3.shapeColor = "white"
 ...
 e1 = createSprite(455, 455, 20, 40);
  ...
  mw4 = createSprite(1440/3.4, 340, 10, 550);
   ...
   MazeSideWalls.setColorEach('green')
  //>maze
  ...
  Lv1Maze.add(m4);
  Lv1Maze.add(m5);
  Lv1Maze.setColorEach('#13a6b0')
  //>subwalls
  
 s1 = createSprite(976, 565, 80, 80);
 s1.shapeColor='#610000'
  s2 = createSprite(970, 150,10,10 );
  s2.shapeColor='#13a6b0'
  s3 = createSprite(490, 360, 125, 10);
  s3.shapeColor='#13a6b0'
   s4 = createSprite(912, 215, 10, 90);
   s4.shapeColor='#13a6b0'
   s5 = createSprite(550, 120 , 10, 90);
   s5.shapeColor='#13a6b0'
   s6 = createSprite(210, 42, 10, 90);
   s6.shapeColor='#13a6b0'
  s6.visible = false;
  s7 = createSprite(625,340,40,40)
  s7.visible = false;
  s7.shapeColor='#13a6b0'
  s8 =createSprite(453,235,50,50)
  s8.visible = false;
  s8.shapeColor='#13a6b0'
  Lv1SubWalls.add(s1);
  Lv1SubWalls.add(s2);
  Lv1SubWalls.add(s3);
  Lv1SubWalls.add(s5);
  ...
  HiddenWalls.add(sew3);
    ...
    se2 = createSprite(1255,340,460,790);
     ...
     easy = createSprite(720,250,170,40);
     easy.visible=false;
     easy.shapeColor="green";
     normal = createSprite(720,320,170,40);
     normal.visible=false;
     normal.shapeColor="green"
     hard = createSprite(720,390,170,40);
     hard.visible=false;
     hard.shapeColor="green"
     extreme = createSprite(720,460,170,40);
     extreme.visible=false;
     extreme.shapeColor="green"
 ...
 // Engine.update(engine);
  ...
  if(gamestate==="rules"){
    button.visible=false;
      ...
      text('reach the goal(hint- it will be in green when opened)',400,550)
    ...
    if(gamestate==="selection"){
      ...
      textSize(31);
      fill("blue");
      text("Please Select your gamemode",520,150);
      fill("red");

      text("Easy",685,260);
      text("Normal",670,330);
      text("Hard",685,400);
      text("Extreme",665,470);
   ...
   s6.visible=false;
    ...
    if (GM > 0) {
      textSize(19);
      fill("red");
      text("Gun bullet -" + CGB, 850, 25+3);
      text("/" + GM, 960, 28);
      fill("black")
      text('PlayerHp = ',750,55);
      fill('white');
      stroke('black');
      strokeWeight(4);
      text('DEAD LEGEND',470,40)
      if (keyWentDown("s")) {
        var bulletr = createSprite(plr.x, plr.y, 30, 5);
       bulletr.shapeColor="red";
        if(bulletr.x>1020){
        bulletr.destroy();
...
function eBullets(n) {
  ...
  if (World.frameCount%30===0) {
    ...
    ebulletsGroup.setColorEach('#00e5ff')
  }
}
if(difficulty===2){
